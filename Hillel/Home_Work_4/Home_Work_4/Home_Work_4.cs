...
namespace Home_Work_4 {
    class Home_Work_4 {
            ...
            for (; ; ) {
                ...
                switch (userSelectTask) {
                    ...
                    case 20:
                        ...
                        break;
                    case 21:
                        WriteLine("Дано 5 вещественных чисел. Найти: \n" +
                            "1) максимальное из вещественных чисел; \n" +
                            "2) минимальное из вещественных чисел; \n" +
                    ...
                    case 23:
                        ...
                        break;
                    case 24:
                        UserInput("Введите стоимость конфет: ", ref a);
                        Task_24(a);
                        break;
                    case 25:
                        UserInput("Введи число A: ", ref a);
         ...
         *
         *cделал 1 таск - 1 метод, и да, без массивов, ограничил пользователя позволив ему вводить только 5 чисел
         и да, с одним оператором цикла(для нахождения минимального и максимального значения)
         все числа записываются в строку, и разделителем чисел является символ  ' | ' 
         бОЖЕЕЕЕЕЕ, МНЕ ТААААААК СТЫДНО, я ребятам в чате ДВЕ НЕДЕЛИ НАЗАД  сказал, что натуральное число n - мол для количества вводимых значений
         Как я  только что понял n - это вариан выбора:
         На примере как я реализовал - если пользователь ввел 1 до выполнится задание а)
         2 - задание б) 
         3 - задание в) 
         возвращаемый параметр - строка с результатом.*/
        ...
        /* 24. Дано вещественное число — цена 1 кг конфет. Вывести стоимость 1.2, 1.4, …, 2 кг конфет.*/
        static void Task_24(int price) {
            for (double i = 1.2; i<=2.0; i +=0.2) {
                WriteLine($"Стоимость {i}кг конфет = {i*price}");
            }
        }

        /*25. Даны целые положительные числа A и B (A < B).
         ...
         */
         static void Task_25(int A, int B) {
            WriteLine($"Выведем в порядке возврастания числа в промежутке между {A} и {B} включительно," +
