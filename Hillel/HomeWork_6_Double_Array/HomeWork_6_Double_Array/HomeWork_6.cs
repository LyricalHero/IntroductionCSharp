    ...
    class HomeWork_6 {
        ...
        private static void LogicProgramm() {
          ...
          //  myEnd_Task();
            ...
            //Task 8
            myTask_8(myNewArr, index1, index2);
        ...
        static void myTask_8(int[,] myArr,int index1, int index2) {
            int max = myArr[0, 0];
            int min = myArr[0,0];
            int mini = 0, minj = 0;
            ...
            for (int i=0;i<index1;i++) {
                for (int j=0; j<index2;j++) {
                    if(max < myArr[i, j]) { max = myArr[i, j];  }       //максимальный элемент
            ...
            WriteLine($"\nМаксимальный элемент массива = {max}, минимальный элемент массива = {min} " +
                $"и координаты этого элемента: [{mini},{minj}] ");
        }
